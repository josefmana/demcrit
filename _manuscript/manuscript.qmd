...
**Consequently, patients with neurological or psychiatric disorder other than PD, including atypical parkinsonism, anticholinergic medication, neurological conditions potentially resulting in cognitive impairment (e.g., stroke, psychoactive substances abuse, epileptic seizures) or gait disorder unresponsive to optimal dopaminergic treatment were excluded from the study. Patients with tremor-dominant PD, rather than predominance of axial symptoms, and better response to dopamingergic treatment were considered preferentially. Nonetheless, axial symptoms such as hypokinetic dysarthria, postural instability and gait freezing/hesitations were not used as a hard exclusion criterion.**
...
**Following variable selection, the optimal probability threshold for predicting Level II PDD classification was determined using the modified, prevalence-adjusted Youden index proposed by @perkins2006 as implemented in the R package _pROC_ [@pROC]. The prevalence adjustment parameter was varied between 0.1 and 0.5 to reflect plausible population PDD prevalence.**
...
**Another limitation stems from the specific subpopulation studied, patient undergoing evaluation for DBS. As discussed above, this group differs from the broader PD population in several important aspect including age distribution and cognitive phenotype. Consequently, neither the estimated PDD rates nor the heuristic screening model derived in this study should be generalized to the broader PD population without further validation. Moreover, given the retrospective nature of our study, some important sample characteristics, such as the rate of motor fluctuations, dyskinesia or wearing-off impact were not available. Larger, more heterogenous cohorts with systematic documentation of these clinical variables would be required to enable approaches such as post-stratification [@deffner2022] to overcome this limitation.**
...
**The prior was implemented using the `horseshoe()` function from the _brms_ package [@brms] with parameters $df = 3$ and $par\_ratio = r$, where $r$ reflect the ratio of expected non-zero parameters to the full predictor set. In the present analysis, $r$ was based on 4 or 5 expected non-zero parameters to reflect the expectation that only a minority of screening variables would meaningfully contribute to prediction. Specifically, @dubois2007 define 4 domains to define cognitive impairment on Level I, while @kulisevsky2024 suggest to add fifth, language domain.**

**The intercept parameter was assigned uniform prior as per _brms_ defaults.**
...
**Following estimation of the reference model, projection predictive variable selection was conducted using the R package _projpred_ [@projpred].**
