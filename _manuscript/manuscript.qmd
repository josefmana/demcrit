...
load_target <- function(x) {
  tar_read_raw(x, store = upstore)
}

data <- load_target("raw_data")
crit <- load_target("pdd_data")$criteria
vars <- load_target("variables")
```
...
desc  <- load_target("sample_description")
rates <- load_target("rate_summaries")
algos <- load_target("algorithms")
pred  <- load_target("demographic_predictors")
conc  <- load_target("concordance_statistics")
kappa <- load_target("kappa_summmaries")
