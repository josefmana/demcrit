...
critnum <- sapply(
  c("mmse", "moca", "smoca", "lvlII"), function(i) {
    subset(crit, group == i) |>
      nrow()
  }
)
...
rates_FAQ9 <- sapply(c("M", "SD", "Md", "minmax"), function(f) do_summary(perc[grepl("FAQ 9", rates$table$IADL)], 2, f))
rates_FAQtot <- sapply(c("M", "SD", "Md", "minmax"), function(f) do_summary(perc[rates$table$IADL == "FAQ > 7"], 2, f))

# Minimum p-value of logistic regressions:
min_p <-
  subset(pred$values, quantity == "p-value")$estimate |>
  min() |>
  do_summary(3, "p")

# List of algorithms:
algos <- list(
  faq_tot = subset(rates$table, IADL == "FAQ > 7")$type,
  faq_9 = subset(rates$table, IADL == "FAQ 9 > 1")$type
)
...
kappas <- lapply(
  set_names(names(algos)), function(o) {
    combn(algos[[o]], 2) |>
      t() |>
      as_tibble() |>
...
Using Level II diagnosis as the gold standard, MoCA-based Level I algorithms,
particularly those using Clock Drawing to assess executive function, demonstrated the highest predictive accuracy. This supports recent proposals to modernise PDD diagnostic frameworks [@kulisevsky2024], favouring MoCA-derived components and more specific, PD-tailored functional assessment tools. In contrast, MMSE-based algorithms consistently underperformed, suggesting limited sensitivity in capturing cognitive deficits typical in PDD.

The diagnostic superiority of the MoCA-based algorithm was evident when using the FAQ total score (accuracy rates of 94–99%). Furthermore, in the algorithm using sMoCA, the raw accuracy was moderate; however, the balanced accuracy (i.e. combined sensitivity and specificity) was high. This suggests that simplified tools such as the sMoCA may be a promising tool for increasing the efficiency of the assessment while maintaining diagnostic accuracy.
...
Nevertheless, the analysis of the algorithm concordance might not be affected by the sample to the same degree allowing for a broader generalization. Future studies should aim to replicate our results on larger population-based PD cohorts. To make this process easier, the code used to generate our results is publicly available and easily applicable to similarly structured data. Furthermore, FAQ is subjective and informant-reliant, which may result in biased data. For future research, we suggest the use of PD-specific questionnaires, such as the Penn Parkinson&#39;s Daily Activities Questionnaire-15 (Brennan et al., 2016) or more objectiveperformance-based methods [@schmitter-edgecombe2020] or questionnaire adaptation including questions regarding gadgets use and digital literacy [@postema2024]. Our results underscore the importance of IADL measurement for the PDD diagnosis. Therefore, we suggest exploring more, reliable tools with high ecological validity. Also, the current study did not explore associations of PDD diagnosis with its neuropsychiatric (e.g. anxiety profile) and biomarker correlates. Future studies would benefit from focusing on these aspects of PDD.

# Conclusions

In sum,our study systematically investigated how varying definitions of impaired cognition and IADL deficit affect PDD diagnostic accuracy. Our study highlights the variability in PDD classification across Level I diagnostic algorithms, significantly influenced by IADL operationalisation and the choice of cognitive screening tools. The findings strongly support the call for a change of the current diagnostic criteria [@kulisevsky2024], favouring the use of MoCA-based components and comprehensive IADL assessments.
