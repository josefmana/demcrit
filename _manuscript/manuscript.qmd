...
critnum <- sapply(c("mmse", "moca", "smoca", "lvlII"), function(i) {
  subset(crit, group == i) |>
    nrow()
})
...
kappas <- lapply(set_names(names(algos)), function(o) {
  combn(algos[[o]], 2) |>
      t() |>
      as_tibble() |>
      `colnames<-`(c("predictor", "reference")) |>
      left_join(conc$table, by = join_by(predictor, reference)) |>
      pull(Kappa_raw)
})
...
kappa_sums <- sapply(names(kappas), function(i) {
