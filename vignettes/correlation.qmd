...
### FAQ item 9
To simulate data for FAQ item 9, we assumed **$\tau$-equivalence** of FAQ (i.e., that all items share the same true score, cf. @trizano-hermosilla2016). We then applied probit link to the FAQ total score of each simulated patient to obtain the probability of a positive response to any item:

$$
p_i = \phi(z_i)
$$ {#eq-p}

where $\phi$ is the standard normal cumulative distribution function and $z_i$ is the standardised FAQ score for simulated patient $i$. The observed FAQ item 9 score was subsequently drawn from a binomial distribution via:
