...
#' followed successively by operationalisation of Global cognitive performance,
#' Attention, Executive Function, Construction, Memory and Language. The
#' dendrogram visualises PDD rate estimated by each algorithm via point size.
#'
#' @param tab A table containing all criteria used as generated by
#'    \code{summarise_rates}.
#' @param top A tibble containing top criteria for predicting PDD
#'    diagnosis by Level II a generated by \code{table_levelII_approximations}.
#'
#' @returns A ggplot object with a dendrogram created
#'    by {ggraph}.
#'
#'@seealso
#' * [summarise_rates()] prepares `tab`.
#' * [table_levelII_approximations()] prepares `top`.
#'
#' @examples
#' \dontrun{
#' # Raw data:
...
#'
#' @export
make_dendrogram <- function(tab, top) {
  ...
  bold <- top |>
    dplyr::filter(stringr::str_detect(part, "Top")) |>
    dplyr::select(tidyselect::starts_with("predictor")) |>
    tidyr::pivot_longer(tidyselect::everything()) |>
    dplyr::pull(value)

  links <- data |>
    dplyr::filter(type %in% bold) |>
    dplyr::select(tidyselect::starts_with("lvl")) |>
    tidyr::pivot_longer(tidyselect::everything()) |>
    dplyr::distinct() |>
    dplyr::pull(value)
  ...
  edge_list <- rbind(
    ...
    dplyr::mutate(
      ...
      highlight = dplyr::if_else(to %in% links, "yes", "no")
    ) |>
  ...
  xcoords <- tibble::tibble(
    Lvl.II = layout |> dplyr::filter(grepl("sMoCA", name)) |> dplyr::pull(x, name = name),
    sMoCA = layout |> dplyr::filter(grepl("Lvl.II", name)) |> dplyr::pull(x, name = name)
  )
  ...
  ggraph::ggraph(layout) +
    ggraph::geom_edge_bend(
      ggplot2::aes(colour = highlight, edge_width = highlight),
      strength = 1
    ) +
    ...
    ggraph::geom_node_point(
      ggplot2::aes(filter = leaf, size = `PDD rate`, colour = group),
      alpha = 0.6
    ) +
    ggraph::geom_node_text(
      ggplot2::aes(label = name, filter = leaf, colour = group),
      angle = 90,
      hjust = 1,
      nudge_y = -0.2
    ) +
    ggraph::scale_edge_colour_manual(values = c("grey", "black")) +
    ggraph::scale_edge_width_manual(values = c(0.6, 0.8)) +
    ggplot2::theme_void() +
    ggplot2::ylim(-1.5, NA) +
