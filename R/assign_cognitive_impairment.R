...
#'
#' Applies cognitive impairment classification based on z-scores computed via
#' the normative regression calculator from Bezdicek et al. (2017), following
#' Level II PD-MCI criteria proposed by Litvan et al. (2012). A patient is
#' classified as cognitively impaired if they have two or more test scores with
#' z < -1.5.
#'
#' @param d0 A list of data, typically generated by \code{prepare_data},
#'   containing test scores.
#' @param calc A character string specifying the file path to the regression
...
#'   \describe{
#'     \item{\code{"ci"}}{A logical vector indicating cognitive impairment
#'     (`TRUE`/`FALSE`).}
#'     \item{\code{"z"}}{A data frame of computed z-scores per cognitive test.}
#'     \item{\code{"both"}}{A data.frame containing both \code{z} (z-scores) and
#'     \code{ci} (impairment status).}
#'   }
#'
#' @seealso [prepare_data()] is a wrapper of this function.
#'
#' @references
#' Bezdicek, O., Sulc, Z., Nikolai, T., Stepankova, H., Kopecek, M., Jech, R.,
#' & Růžička, E. (2017). A parsimonious scoring and normative calculator for the
#' Parkinson’s disease mild cognitive impairment battery.
#' *The Clinical Neuropsychologist, 31*(6-7), 1231-1247.
#' \doi{10.1080/13854046.2017.1293161}
#'
#' Litvan, I., Goldman, J. G., Tröster, A. I., Schmand, B. A., Weintraub, D.,
#' Petersen, R. C., ... & Emre, M. (2012). Diagnostic criteria for mild
#' cognitive impairment in Parkinson's disease: Movement Disorder Society Task
#' Force guidelines. *Movement disorders, 27*(3), 349-356.
