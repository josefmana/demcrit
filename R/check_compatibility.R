...
#'
#' @param d1 A data frame generated by \code{import_item_data}, containing
#'   item-level data.
#' @param d2 A data frame generated by \code{import_redcap_data}, containing
#'   REDCap data.
#'
#' @returns If discrepancies are found, an informative message is printed and
#'   the function stops execution (e.g., via \code{stop()}). This function is
#'   intended to be used inside \code{prepare_data()} as a consistency check.
#'
#' @seealso
#' * [prepare_data()] is a wrapper of this function.
#' * [import_item_data()] prepares `d1`.
#' * [import_redcap_data()] prepares `d2`.
#'
#' @examples
#' \dontrun{
...
check_compatibility <- function(d1, d2) {
  ...
  d1 |>
    dplyr::select(id, surname, firstname, assdate) |>
