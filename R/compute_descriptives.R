...
#'
#' Alternatively, a path to a CSV file (semicolon-delimited) containing this
#'   information.
#'
#' @returns A list containing:
#'   \describe{
#'     \item{\code{table}}{A tibble with raw descriptive statistics.}
#'     \item{\code{gtable}}{An APA-style formatted {gt} table summarizing
#'     selected variables.}
#'   }
#'
#' @seealso [prepare_data()] prepares `d0`.
#'
#' @examples
#' \dontrun{
...
compute_descriptives <- function(d0, vois) {
  ...
  if (ncol(v) > 4 && any(!is.na(v$note))) {
    ...
    gtab <- gtab |>
      gt::tab_source_note(
        gt::html(glue::glue("<i>Note.</i> {text}, all percentages were calculated from the whole sample."))
      )
  }
